...
可选人工验证入口默认关闭。需要远程处理票星球官方人机验证时，在反向代理配置
HTTPS 后增加：

```ini
Environment=PIAOXINGQIU_OPERATOR_PUBLIC_URL=https://你的域名/verify
Environment=PIAOXINGQIU_OPERATOR_LISTEN=127.0.0.1:8765
```

反向代理需将 `/verify/` 去掉前缀后转发至 `127.0.0.1:8765`。入口令牌仅可兑换
一次、5 分钟失效，只显示并操作原 Playwright 页面中的官方验证区域；不暴露
Cookie、认证头、证件信息或完整订单页。未配置时，排队与验证状态仍会被识别并
保留原页面，但只发送提示。
...
搜索后直接发送搜索序号；存在多个场次时再发送场次序号。搜索、登录和绑定配置
都是分步流程，发送 `取消` 即可退出。
账号登录独立于抢票任务，可以先登录，再创建任务并绑定账号。
...
绑定流程按顺序引导选择票档、看台、选座策略、人数或数量，以及票星球账号中已有的观演人。
看台限制按官方看台名精确匹配；多个指定看台不分先后，并在其范围内继续使用原选座规则。
尚未开售且官方未下发看台时，可提前输入官方精确看台名；开售后任一名称无法匹配时，
自动按不限看台执行。
选座项目按实际入场人数配置，固定套票和普通票可以共同作为优先级；非选座项目按
官方商品张数配置，固定套票的一张就是一个套票商品，且必须单独选择。一单一证选择一位；
一票一证按实际入场人数选择观演人。配置只有在发送
...
程序同时读取官方促销 ID、适用票档和版本哈希，再按官方优惠金额生成创建参数。
创建请求由 booking 页面内置客户端生成动态风控头。

`速度优先`不加载位置模型，并在同等级合法候选中随机选择；`位置优先`复用同一候选队列，并按场馆几何排序。
存在官方 `innerTemplate` 的场馆按球场处理，优先中线附近看台和场地中心附近座位；
其他场馆按剧场处理，先靠全场中线，再靠前排。同一场次的多个账号在提交前原子占用
不同座位，库存计算和网络请求仍保持异步并行。
...
- 每次只开放一个确定业务体的逻辑提交租约；业务体变化、并发提交和租约外请求均拦截。
- 官方排队、人机验证和 `mode=limiting` 可在同一租约内重发完全相同的业务体。
- `CREATED` 和 `UNKNOWN` 状态阻止重复创建。
- 座位被抢或临时票据失效时，程序刷新实时库存并重新选择。
- `mode=limiting` 保留原订单并每 3 秒重发，最多重试 3 次；不带已知风险信号的
  `33000000` 不再被猜测成座位冲突。
- 排队或人机验证期间保留 booking 页面、原请求体和订单保护，超时无终态则标记
  `UNKNOWN` 并要求人工核对。
- `28217767` 套票规则拒绝会停止当前“任务 + 账号”绑定，不影响该账号的其他任务。
