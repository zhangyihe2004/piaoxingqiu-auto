...
def build_order_config(
    ...
    return AccountRunConfig(
        ...
        purchase=PurchaseConfig(
            ...
            plans=tuple(plan["plan_name"] for plan in plans),
            plan_ids=tuple(plan["seat_plan_id"] for plan in plans),
            stand_names=tuple(stand["stand_name"] for stand in stands),
            quantity=quantity,
            real_name_mode=task["real_name_mode"],
            audiences=people,
            position_priority=bool(binding["position_priority"]),
        ),
